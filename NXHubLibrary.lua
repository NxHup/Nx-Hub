...
function NXHub:Notify(config)
    ...
    TitleLbl.Size = UDim2.new(1, -20, 0, 20); TitleLbl.Position = UDim2.new(0, 12, 0, 6)
    TitleLbl.BackgroundTransparency = 1; TitleLbl.Text = "🔔 " .. title
    TitleLbl.TextColor3 = Color3.fromRGB(0, 240, 255); TitleLbl.Font = Enum.Font.GothamBold
    TitleLbl.TextSize = 13; TitleLbl.TextXAlignment = Enum.TextXAlignment.Left
    TitleLbl.ZIndex = 201; TitleLbl.Parent = NotifFrame

    local ContentLbl = Instance.new("TextLabel")
    ContentLbl.Size = UDim2.new(1, -24, 0, 24); ContentLbl.Position = UDim2.new(0, 12, 0, 26)
    ContentLbl.BackgroundTransparency = 1; ContentLbl.Text = content
    ContentLbl.TextColor3 = Color3.fromRGB(220, 225, 240); ContentLbl.Font = Enum.Font.GothamMedium
...
function NXHub:CreateWindow(config)
    ...
    if not logoAsset then
        ...
        TextFallback.Size = UDim2.new(1, 0, 1, 0); TextFallback.BackgroundTransparency = 1
        TextFallback.Text = "⚡ NX"; TextFallback.TextColor3 = Color3.fromRGB(0, 240, 255)
        TextFallback.Font = Enum.Font.GothamBold; TextFallback.TextSize = 13
        TextFallback.ZIndex = 998; TextFallback.Parent = ToggleBtn
    end
    ...
    Topbar.BackgroundTransparency = 1; Topbar.Parent = WindowBg
    ...
    local TitleLbl = Instance.new("TextLabel")
    TitleLbl.Size = UDim2.new(0, 240, 1, 0); TitleLbl.Position = UDim2.new(0, 18, 0, 0)
    TitleLbl.BackgroundTransparency = 1
    TitleLbl.Text = "<font color=\"#00f0ff\" size=\"17\"><b>" .. titleText .. "</b></font>   <font color=\"#ff00b7\" size=\"11\">" .. subTitleText .. "</font>"
    TitleLbl.RichText = true; TitleLbl.Font = Enum.Font.GothamBold
    TitleLbl.TextXAlignment = Enum.TextXAlignment.Left; TitleLbl.Parent = Topbar

    -- FPS & Ping Pill
    local StatusPill = Instance.new("Frame")
    StatusPill.Name = "StatusPill"; StatusPill.Size = UDim2.new(0, 160, 0, 26)
    StatusPill.Position = UDim2.new(1, -295, 0.5, -13)
    StatusPill.BackgroundColor3 = Color3.fromRGB(15, 18, 28); StatusPill.Parent = Topbar
    ...
    FpsLbl.Size = UDim2.new(0.5, -2, 1, 0); FpsLbl.Position = UDim2.new(0, 4, 0, 0)
    FpsLbl.BackgroundTransparency = 1; FpsLbl.Text = "⚡ -- FPS"
    FpsLbl.TextColor3 = Color3.fromRGB(0, 240, 255); FpsLbl.Font = Enum.Font.GothamBold
    FpsLbl.TextSize = 10; FpsLbl.Parent = StatusPill

    local PingLbl = Instance.new("TextLabel")
    PingLbl.Size = UDim2.new(0.5, -2, 1, 0); PingLbl.Position = UDim2.new(0.5, 0, 0, 0)
    PingLbl.BackgroundTransparency = 1; PingLbl.Text = "📶 -- ms"
    PingLbl.TextColor3 = Color3.fromRGB(0, 255, 150); PingLbl.Font = Enum.Font.GothamBold
    PingLbl.TextSize = 10; PingLbl.Parent = StatusPill

    task.spawn(function()
        local lastFrameTime = os.clock(); local frameCount = 0; local fps = 60
        local renderConn
        ...
        while task.wait(0.5) do
            if not ScreenGui or not ScreenGui.Parent then if renderConn then renderConn:Disconnect() end break end
            local ping = 0
    ...
    ControlsFrame.Size = UDim2.new(0, 120, 1, -4); ControlsFrame.Position = UDim2.new(1, -125, 0, 2)
    ControlsFrame.BackgroundTransparency = 1; ControlsFrame.Parent = Topbar
    ...
    Sidebar.Name = "Sidebar"; Sidebar.Size = UDim2.new(0, 155, 1, -54)
    Sidebar.Position = UDim2.new(0, 0, 0, 46); Sidebar.BackgroundTransparency = 1
    Sidebar.BorderSizePixel = 0; Sidebar.ScrollBarThickness = 2
    Sidebar.ScrollBarImageColor3 = Color3.fromRGB(0, 240, 255)
    Sidebar.AutomaticCanvasSize = Enum.AutomaticSize.Y
    Sidebar.CanvasSize = UDim2.new(0, 0, 0, 0); Sidebar.Parent = WindowBg

    local TabListLayout = Instance.new("UIListLayout"); TabListLayout.Padding = UDim.new(0, 5); TabListLayout.Parent = Sidebar
    local SidebarPadding = Instance.new("UIPadding")
    SidebarPadding.PaddingTop = UDim.new(0, 8); SidebarPadding.PaddingLeft = UDim.new(0, 10)
    SidebarPadding.PaddingRight = UDim.new(0, 10); SidebarPadding.PaddingBottom = UDim.new(0, 8)
    ...
    ContentContainer.Size = UDim2.new(1, -165, 1, -56)
    ...
    CloseBtn.MouseButton1Click:Connect(function()
        ShowDialog({
            ...
            Buttons = {
                { Title = "ปิดสคริปต์", Callback = function()
    ...
    function WindowObj:AddTab(tabConfig)
        ...
        TabPage.Size = UDim2.new(1, -5, 1, 0); TabPage.BackgroundTransparency = 1
        TabPage.ScrollBarThickness = 3; TabPage.ScrollBarImageColor3 = Color3.fromRGB(0, 240, 255)
        TabPage.Visible = false; TabPage.AutomaticCanvasSize = Enum.AutomaticSize.Y
        TabPage.CanvasSize = UDim2.new(0, 0, 0, 0); TabPage.Parent = ContentContainer

        local PageLayout = Instance.new("UIListLayout")
        PageLayout.Padding = UDim.new(0, 10); PageLayout.SortOrder = Enum.SortOrder.LayoutOrder
        ...
        table.insert(Tabs, { Btn = TabBtn, Page = TabPage, Indicator = Indicator, TabText = TabText })

        if #Tabs == 1 then
            TabPage.Visible = true; Indicator.Visible = true
            TabBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 240)
            TabText.TextColor3 = Color3.fromRGB(255, 255, 255)
        end
        ...
        local function getNextOrder() currentLayoutOrder = currentLayoutOrder + 1; return currentLayoutOrder end
        ...
        function TabObj:AddToggle(id, toggleConfig)
            ...
            function toggleObj:OnChanged(func)
                changedFunc = func
                if changedFunc then task.spawn(function() pcall(function() changedFunc(state) end) end) end
            end
            function toggleObj:SetValue(val)
                state = (val == true); toggleObj.Value = state
        ...
        function TabObj:AddSlider(id, sliderConfig)
            ...
            local changedFunc = nil; local sliding = false
            local sliderObj = { Value = val }
            function sliderObj:OnChanged(func)
                changedFunc = func
                if changedFunc then task.spawn(function() pcall(function() changedFunc(val) end) end) end
            end
            function sliderObj:SetValue(v)
                val = math.clamp(v, min, max); sliderObj.Value = val
        ...
        function TabObj:AddDropdown(id, dropConfig)
            ...
            if hasDesc then
                local D = Instance.new("TextLabel"); D.Size = UDim2.new(1, -150, 0, 18); D.Position = UDim2.new(0, 16, 0, 28); D.BackgroundTransparency = 1; D.Text = desc; D.TextColor3 = Color3.fromRGB(140, 150, 175); D.Font = Enum.Font.GothamMedium; D.TextSize = 10; D.TextXAlignment = Enum.TextXAlignment.Left; D.Parent = Frame
            end
            ...
            if isMulti then
                if typeof(default) == "table" then
                    for k, v in pairs(default) do if typeof(k) == "string" then selectedMulti[k] = v else selectedMulti[v] = true end end
                elseif typeof(default) == "number" and values[default] then selectedMulti[values[default]] = true
                elseif typeof(default) == "string" then selectedMulti[default] = true end
            end

            local count = 0; for k, v in pairs(selectedMulti) do if v then count = count + 1 end end
            local initText = isMulti and (count .. " Selected ▼") or (tostring(selected) .. " ▼")

            local DropBtn = Instance.new("TextButton"); DropBtn.Size = UDim2.new(0, 130, 0, 28); DropBtn.Position = UDim2.new(1, -142, 0, hasDesc and 14 or 8); DropBtn.BackgroundColor3 = Color3.fromRGB(32, 37, 56); DropBtn.Text = initText; DropBtn.TextColor3 = Color3.fromRGB(0, 240, 255); DropBtn.Font = Enum.Font.GothamBold; DropBtn.TextSize = 11; DropBtn.Parent = Frame
            ...
            local optionButtons = {}; local changedFunc = nil
            local dropObj = { Value = isMulti and selectedMulti or selected, Values = values }
            function dropObj:OnChanged(func)
                changedFunc = func
                if changedFunc then task.spawn(function() pcall(function() changedFunc(dropObj.Value) end) end) end
            end
            function dropObj:SetValue(val)
                if isMulti and typeof(val) == "table" then
                    selectedMulti = val; dropObj.Value = val
                    local cnt = 0; for k, v in pairs(selectedMulti) do if v then cnt = cnt + 1 end end
                    ...
                    selected = val; dropObj.Value = val; DropBtn.Text = tostring(selected) .. " ▼"
                    for opt, optBtn in pairs(optionButtons) do
            ...
            for _, opt in ipairs(values) do
                ...
                OptBtn.MouseButton1Click:Connect(function()
                    if isMulti then
                        ...
                        local cnt = 0; for k, v in pairs(selectedMulti) do if v then cnt = cnt + 1 end end
                        DropBtn.Text = cnt .. " Selected ▼"; dropObj.Value = selectedMulti
                        if changedFunc then pcall(function() changedFunc(selectedMulti) end) end
                        SaveManager:SaveConfig("default")
                    else dropObj:SetValue(opt) end
                end)
            end
        ...
        function TabObj:AddButton(btnConfig)
            ...
            local Btn = Instance.new("TextButton"); Btn.Size = UDim2.new(1, 0, 0, hasDesc and 56 or 44); Btn.BackgroundColor3 = Color3.fromRGB(22, 26, 40); Btn.Text = ""; Btn.AutoButtonColor = false; Btn.LayoutOrder = getNextOrder(); Btn.Parent = TabPage
            ...
            local TitleLabel = Instance.new("TextLabel"); TitleLabel.Size = UDim2.new(1, -110, 0, hasDesc and 22 or 44); TitleLabel.Position = UDim2.new(0, 16, 0, hasDesc and 8 or 0); TitleLabel.BackgroundTransparency = 1; TitleLabel.Text = title; TitleLabel.TextColor3 = Color3.fromRGB(240, 245, 255); TitleLabel.Font = Enum.Font.GothamBold; TitleLabel.TextSize = 12; TitleLabel.TextXAlignment = Enum.TextXAlignment.Left; TitleLabel.Parent = Btn
            if hasDesc then
                local D = Instance.new("TextLabel"); D.Size = UDim2.new(1, -110, 0, 18); D.Position = UDim2.new(0, 16, 0, 28); D.BackgroundTransparency = 1; D.Text = desc; D.TextColor3 = Color3.fromRGB(140, 150, 175); D.Font = Enum.Font.GothamMedium; D.TextSize = 10; D.TextXAlignment = Enum.TextXAlignment.Left; D.Parent = Btn
            end
            local ActionBadge = Instance.new("Frame"); ActionBadge.Size = UDim2.new(0, 80, 0, 26); ActionBadge.Position = UDim2.new(1, -94, 0.5, -13); ActionBadge.BackgroundColor3 = Color3.fromRGB(0, 190, 255); ActionBadge.Parent = Btn
            local BadgeCorner = Instance.new("UICorner"); BadgeCorner.CornerRadius = UDim.new(0, 8); BadgeCorner.Parent = ActionBadge
            local BadgeText = Instance.new("TextLabel"); BadgeText.Size = UDim2.new(1, 0, 1, 0); BadgeText.BackgroundTransparency = 1; BadgeText.Text = "▶ Click"; BadgeText.TextColor3 = Color3.fromRGB(10, 14, 24); BadgeText.Font = Enum.Font.GothamBold; BadgeText.TextSize = 11; BadgeText.Parent = ActionBadge

            Btn.MouseEnter:Connect(function() TweenService:Create(Btn, TweenInfo.new(0.2), { BackgroundColor3 = Color3.fromRGB(30, 36, 56) }):Play(); TweenService:Create(Stroke, TweenInfo.new(0.2), { Transparency = 0.2 }):Play(); TweenService:Create(ActionBadge, TweenInfo.new(0.2), { BackgroundColor3 = Color3.fromRGB(0, 230, 255) }):Play() end)
            Btn.MouseLeave:Connect(function() TweenService:Create(Btn, TweenInfo.new(0.2), { BackgroundColor3 = Color3.fromRGB(22, 26, 40) }):Play(); TweenService:Create(Stroke, TweenInfo.new(0.2), { Transparency = 0.7 }):Play(); TweenService:Create(ActionBadge, TweenInfo.new(0.2), { BackgroundColor3 = Color3.fromRGB(0, 190, 255) }):Play() end)
        ...
        function TabObj:AddInput(id, inputConfig)
            ...
            if hasDesc then
                local D = Instance.new("TextLabel"); D.Size = UDim2.new(1, -160, 0, 18); D.Position = UDim2.new(0, 16, 0, 28); D.BackgroundTransparency = 1; D.Text = desc; D.TextColor3 = Color3.fromRGB(140, 150, 175); D.Font = Enum.Font.GothamMedium; D.TextSize = 10; D.TextXAlignment = Enum.TextXAlignment.Left; D.Parent = Frame
            end
            ...
            function inputObj:SetValue(v)
                v = tostring(v or ""); inputObj.Value = v; TextBox.Text = v
